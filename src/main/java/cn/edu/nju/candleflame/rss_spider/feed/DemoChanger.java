...
public class DemoChanger implements FeedChanger {
	...
	private static final RunningLog LOGGER = RunningLog.getLog(DemoChanger.class);
	@Autowired
	...
	public RssDocument analysis() {
		...
		String url1 = "https://www.javadoop.com";
		Future<String> baiduContent = fetchHtmlService.get(url1, null);

		String content = "";
		try {
			content = baiduContent.get();
		} catch (InterruptedException | ExecutionException e) {
			LOGGER.error(e.getLocalizedMessage());
		}

		Item baidu = new Item("首页",url1, content);

		List<Item> itemList = Arrays.asList(item1,item2,baidu);
